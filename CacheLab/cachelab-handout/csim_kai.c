...
int E = 0;
int b = 0;
int nw_ins_idx = 0;     // 当前指令索引，recentlyUsed用它来记录最近使用某个缓存行的指令索引，以确定组内的lru
int miss = 0;
int hit = 0;
int eviction = 0;
...
int LRU(MemAddr addr, int groupIdx){
    int minInsIdx = recentlyUsed[groupIdx];
    int resSetIdx = groupIdx;
    for(int i = 1; i < E; i++){
        int nw_setIdx = groupIdx + i;
        if(recentlyUsed[nw_setIdx] < minInsIdx){
            minInsIdx = recentlyUsed[nw_setIdx];
            resSetIdx = nw_setIdx;
        }
    }
    return resSetIdx;
...
void processAddr(char * start, int len){
    ...
    for(int i = 0; i < E; i++){
        if(cacheLineSet[groupIdx+i].valid == 0){
            ...
            hitFlag = 1;
            missFlag = 0;
            recentlyUsed[groupIdx+i] = nw_ins_idx;
            break;
        }
        else{
            continue;
    ...
    recentlyUsed[victim] = nw_ins_idx;
...
int main(int argc, char ** argv)
{
    ...
    recentlyUsed = (int*)malloc(setSize * sizeof(int));
    memset(recentlyUsed, -1, setSize * sizeof(int));
    FILE * fp;
